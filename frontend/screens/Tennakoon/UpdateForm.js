...
function FormChildQuestion({ questionNo, setInfoshow, openModal }) {
  return (
    <View style={{ zIndex: 1, flexDirection: 'row' }}>
      ...
      >
        <Animated.Text style={[styles.text, { opacity: openModal ? 0.1 : 1 }]}>
...
export default function UpdateForm({ route, navigation }) {
  let { average, country, questionSet, headCarbon, results } = route.params;
  const arrow = {
  ...
  const [modResults, setModResults] = useState([...results]);
  const [modHeadCarbon, setModHeadCarbon] = useState(headCarbon);
  ...
  useEffect(() => {
    ...
    for (var i = 0; i < results.length; i++) {
      temp = temp + Number.parseFloat(modResults[i]);
    }

    setModHeadCarbon(temp + Number.parseFloat(average));
  }, [modResults]);
  ...
  const getCustomVal = (carbonForAnswer) => {
    ...
    let modRes = modResults.map((rs, i) => {
      if (i === questionSet[questionSetIndex]) return carbonForAnswer;
      else return modResults[i];
    });
    setModResults(modRes);
    setNextEnable(true);
  };
  ...
  const answerSelect = (ans) => {
    ...
    let carbonForAnswer =
      (average *
        QandA[questionSet[questionSetIndex]].IMPACT[
          QandA[questionSet[questionSetIndex]].ANS.indexOf(ans)
        ]) /
      100;

    if (carbonForAnswer < results[questionSet[questionSetIndex]]) {
      setArrowType(arrow.greendown);
    } else if (carbonForAnswer > results[questionSet[questionSetIndex]]) {
      setArrowType(arrow.redup);
    } else if (carbonForAnswer === results[questionSet[questionSetIndex]]) {
      setArrowType({});
    }
  ...
  const nextQues = () => {
    if (questionSetIndex === questionSet.length - 1) {
      navigation.navigate('Finetune', {
        average: average,
        country: country,
        results: modResults,
        headCarbon: modHeadCarbon,
      });
    } else {
      setCustomValue('');
      setArrowType({});
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Block flex style={{ opacity: openModal ? 0.1 : 1 }}>
        <ImageBackground
          source={QandA[questionSet[questionSetIndex]].BKIMG}
        ...
        <Block space="between" style={styles.padded}>
          <Block middle style={styles.button}>
            ...
            >
              {Number.parseFloat(modHeadCarbon).toFixed(2)}
            </Animated.Text>
            <Text
              style={{
                fontFamily: 'montserrat-regular',
      ...
      >
        <FormChildQuestion
          questionNo={questionSet[questionSetIndex]}
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
        <Button
          disabled={questionSetIndex === 0 ? true : false}
          onPress={backQues}
          round
