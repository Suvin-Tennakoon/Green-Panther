...
const ACard = ({ data, isView = true, onDelete, ...props }) => {
  ...
  return (
    ...
    >
      ...
      <View style={styles.articleDetails}>
        ...
        <Text numberOfLines={9} style={styles.title}>
          {data.title}
        </Text>
        <View style={styles.btnContainer}>
          {isView ? (
            <Button
              title="More details"
              color="#59A167"
              onPress={() => navigation.navigate('Article Details', { data: data})}
            />
          ) : (
            <View style={styles.btnContainerInside}>
              <Button
                title="Update"
                color="#59A167"
                onPress={() => navigation.navigate('Update Article', {data: data })}
              />
              <Button
                title="Delete"
                color="#111"
